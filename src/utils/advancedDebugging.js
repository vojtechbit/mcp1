...
const consoleLoggingEnabled = process.env.ADVANCED_DEBUG_CONSOLE !== 'false';
const fileLoggingTarget = process.env.ADVANCED_DEBUG_FILE;

const debugSinks = new Set();
let fileStream;
...
function consoleSink({ timestamp, level, message, payload }) {
...
function ensureFileStream(targetPath) {
  if (fileStream) {
    return fileStream;
  }
  ...
  fileStream = fs.createWriteStream(absolutePath, { flags: 'a' });
  return fileStream;
}

function createFileSink(targetPath) {
  return ({ timestamp, level, message, payload }) => {
    try {
      const stream = ensureFileStream(targetPath);
      stream.write(
        `${JSON.stringify({
          timestamp,
          level,
      ...
      console.error('Advanced debug file sink failed', error);
    }
  };
}

function registerDebugSink(sink) {
  if (typeof sink !== 'function') {
    throw new TypeError('registerDebugSink expects a function');
  }
...
function clearDebugSinks() {
  debugSinks.clear();
}

function emitToSinks(entry) {
  if (debugSinks.size === 0) {
    return;
  }
  ...
  for (const sink of debugSinks) {
    try {
      ...
      console.error('Advanced debug sink failed', error);
    }
  }
}
...
if (consoleLoggingEnabled) {
  registerDebugSink(consoleSink);
}

if (fileLoggingTarget) {
  registerDebugSink(createFileSink(fileLoggingTarget));
}
